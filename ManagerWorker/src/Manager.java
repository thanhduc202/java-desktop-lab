    ...
    public static void changeSalary(ArrayList<Worker> lw, ArrayList<History> lh, int status) {
        ...
        if (worker == null) {
            ...
            if (status == 1) {
                ...
                lh.add(new History("UP", getCurrentDate(date), worker.getId(),
                ...
                Date date = Calendar.getInstance().getTime();
                lh.add(new History("DOWN", getCurrentDate(date), worker.getId(),
                        worker.getName(), worker.getAge(), salaryUpdate,
                        worker.getWorkLocation()));
            }
...
//        Calendar calendar = Calendar.getInstance();
    ...
    public static void display(ArrayList<Worker>lw){
         System.out.printf("%-5s%-15s%-5s%-10s%-20s\n", "Code", "Name", "Age",
                "Salary", "work location");
         for (Worker worker : lw) {
             System.out.printf("%-5s%-15s%-5s%-10s%-20s", worker.getId(),
